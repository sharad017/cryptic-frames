...
export default function HeroSlideshow({ images }: { images: string[] }) {
  ...
  const getStyle = (src: string): React.CSSProperties => {
    const key = src.replace("/images/", "");
    return getFocalStyle(focal, key, isMobile);
  };
    ...
    <>
      {prev !== null && (
        <img key={`prev-${prev}`} src={images[prev]} alt=""
          ...
          style={{ ...getStyle(images[prev]), opacity: transitioning ? 0 : 1, transition: "opacity 1.8s cubic-bezier(0.4,0,0.2,1)", zIndex: 1 }} />
      )}
      <img key={`curr-${current}`} src={images[current]} alt=""
        className="absolute inset-0 w-full h-full object-cover"
        style={{ ...getStyle(images[current]), zIndex: 2 }} />
        ...
        <div className="absolute bottom-24 left-1/2 -translate-x-1/2 flex gap-2 z-10">
          {images.map((_, i) => (
            <button key={i} onClick={() => { setPrev(current); setCurrent(i); }}
              ...
              style={{ width: i === current ? "20px" : "5px", height: "5px", background: i === current ? "var(--accent)" : "rgba(255,255,255,0.2)" }} />
          ))}
        </div>
      )}
    </>
